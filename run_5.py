...
parser.add_argument('--max_epochs', type=int, default=10)
parser.add_argument('--n_clustering', type=int, default=2)
    ...
    def train_step(self, heads, tails, rels, tc=None):
        with tf.GradientTape() as tape:
            ...
            for step in range(1, self.hparams.max_steps + 1):
                ...
                #DEBUG['h_con.max'].append(tf.reduce_max(tf.sqrt(tf.reduce_sum(tf.square(hidden_con), axis=1))))

            self.model.past_hidden_uncon = hidden_uncon

            #DEBUG['h_uncon.max'] = tf.reduce_max(tf.sqrt(tf.reduce_sum(tf.square(hidden_uncon), axis=2)))
...
def str_debug():
    def _str(v):
        if isinstance(v, list):
            return ', '.join('{:.3f}'.format(e) for e in v)
        else:
            return '{:.3f}'.format(v)

    return ', '.join('{} {}'.format(k, _str(v))
