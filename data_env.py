...
class Graph(object):
    ...
    def get_selected_edges(self, sampled_edges):
        """ `sampled_edges`: n_sampled_edges x 2 ( int32, outputs[i] = (eg_idx, edge_id) )
        """
        sampled_edges = sampled_edges.numpy()
    ...
    def get_attended_nodes(self, node_attention, max_nodes):
    ...
    def get_attended_edges(self, attended_nodes):
        ...
        for eg_idx in range(batch_size):
            ...
            attended_eid = np.sort(self.edges[:, 0][mask])
    ...
    def get_selfloop_and_backtrace(self, attended_nodes, attended_node_attention,
                                   max_backtrace_edges, step, backtrace_decay=1.):
    ...
    def get_union_edges(self, selected_edges, selfloope_edges, backtrace_edges):
    ...
    def store_transitions(self, selected_edges, selected_trans_att, step):
        """ selected_edges: n_selected_edges x 6 (eg_idx, vi, vj, rel, idx_vi, idx_vj)
