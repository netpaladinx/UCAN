...
class Graph(object):
    ...
    def get_attended_edges(self, attended_nodes, tc=None):
        ...
        for eg_idx in range(batch_size):
            ...
            mask = np.in1d(self.edges[:, 1], attended_vi)
            attended_eid = self.edges[:, 0][mask]  # already sorted
            attended_edges.append(np.stack([np.repeat(np.array(eg_idx, dtype='int32'), attended_eid.shape[0]),
                                            attended_eid], axis=1))
        attended_edges = np.concatenate(attended_edges, axis=0)

        if tc is not None:
            tc['att_e'] += time.time() - t0
        return attended_edges  # n_attended_edges x 2 ( attended_edges[i] = [eg_idx, edge_id], sorted by eg_idx, edge_id )
    ...
    def sample_edges(self, candidate_edges, edges_logits, max_edges, tc=None):
        """ candidate_edges: n_candidate_edges x 2, (eg_idx, edge_id)
            edges_logits: n_full_edges
        """
        if tc is not None:
            t0 = time.time()

        edges_logits = edges_logits.numpy()
        eg_idx, edge_id = candidate_edges[:, 0], candidate_edges[:, 1]  # n_candidate_edges, n_candidate_edges
        logits = edges_logits[edge_id]  # n_candidate_edges

        eps = 1e-20
        loglog_u = - np.log(- np.log(np.random.uniform(size=(len(candidate_edges),)) + eps) + eps)  # n_candidate_edges
        loglog_u = np.array(loglog_u, dtype='float32')
        logits = logits + loglog_u
        ...
        for t in range(batch_size):
            mask = (eg_idx == t)
            y = logits[mask]  # n_candidate_edges_t where eg_idx == t
            e_id = edge_id[mask]  # n_candidate_edges_t where eg_idx == t

            k = min(max_edges, len(y))
            iy = np.array(np.argsort(y)[-k:][::-1], dtype='int32')  # n_sampled_edges_t
            e = e_id[iy]
            n_e = len(iy)
            eg = np.repeat(np.array(t, dtype='int32'), n_e)
            eg_e = np.stack([eg, e], axis=1)  # n_sampled_edges_t x 2
            eg_iy = np.stack([eg, iy], axis=1)  # n_sampled_edges_t x 2

            y_indices.append(eg_iy)
            sampled_edges.append(eg_e)

        y_indices = np.concatenate(y_indices, axis=0)
        sampled_edges = np.concatenate(sampled_edges, axis=0)

        if tc is not None:
            tc['sample'] += time.time() - t0
        # loglog_u: n_candidate_edges
        # y_indices: n_sampled_edges x 2, (eg_idx, idx_for_y)
        # sampled_edges: n_sampled_edges x 2, (eg_idx, edge_id)
        return loglog_u, y_indices, sampled_edges
