...
class Sampler(keras.Model):
    ...
    def call(self, _, candidate_edges=None, loglog_u=None, sampled_edges=None, training=None, tc=None):
        """ inputs: None
            candidate_edges: (pd.DataFram) n_candidate_edges x 5, (eg_idx, edge_id, vi, vj, rel) sorted by (eg_idx, vi)
        ...
        idx_and_vi = candidate_edges[['eg_idx', 'vi']].sort_values(['eg_idx', 'vi']).values
        _, idx_vi = np.unique(idx_and_vi, axis=0, return_inverse=True)
        idx_vi = np.array(idx_vi, dtype='int32')
...
class Model(object):
    ...
    def flow(self, hidden_uncon, hidden_con, node_attention, step, time_cost=None):
        ...
        candidate_edges = self.graph.get_candidate_edges(attended_nodes,
                                                         tc=time_cost['graph'] if time_cost else None)  # n_candidate_edges x 2
        # sampled_edges: n_attended_edges x 2, (eg_idx, edge_id)
        # edges_y: n_attended_edges
        loglog_u, y_indices, sampled_edges = self.graph.sample_edges(candidate_edges, self.sampler.edges_logits,
